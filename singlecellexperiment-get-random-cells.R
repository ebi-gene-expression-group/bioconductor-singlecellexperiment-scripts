...
# Given an R scater object, fetch a set of random cells. This needs to be random
...
# Parse the arguments
opt <- wsc_parse_args(option_list, mandatory = c('input_object_file', 'output_text_file', 'n_cells'))
...
# Select random genes
random_cells <- sample(colnames(SingleCellExperiment), opt$n_cells)

# Write to file

writeLines(con = opt$output_text_file, random_cells)
